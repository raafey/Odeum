	...
	<div class="table">
		...
		<table class="table table-hover">
			...
			<tbody>
				...
				<?php $rating_lower = $_GET["rating"];
				$rating_floor = floor($rating_lower);

				if ($rating_lower == $rating_floor) {
					$rating_upper = $rating_lower + 1;
					$sql = "SELECT * FROM odeum.event WHERE rating BETWEEN '$rating_lower' AND '$rating_upper'";
				}

				else {
					$sql = "SELECT * FROM odeum.event WHERE rating='$rating_lower'";
				}
